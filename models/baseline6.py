...
# ══ 2. BATCH UNPACKERS ══
#
# Canonical unpackers live in src.data.unpackers. B6's Stage A routes each
...
def train_test(cfg: DictConfig) -> None:
    ...
    # ── Logging ──────────────────────────────────────────────────────────
    run_id = Trainer.next_run_id("baseline6")
    trainer = Trainer("baseline6", run_id, device=device)
    ...
    # across them (a fine-tune epoch only overwrites the probe's best).
    ...
    # )
    # AdamW equivalent — probe trains the MLP head only (AdamW-scale warmup_lr).
    ...
    trainer.run_stage(
        ...
        stage="B-probe", desc="B6-B-probe", **stage_b_kwargs,
    )
    ...
    #     weight_decay=weight_decay_b,
    # )
    # AdamW equivalent — differential LR groups preserved; AdamW-scale temporal
    # lr (1e-4 fine-tune of the pretrained LSTM), head at ×head_mult.
    ...
    best_f1_b = trainer.run_stage(
        ...
        stage="B-ft", desc="B6-B-ft", **stage_b_kwargs,
    )
