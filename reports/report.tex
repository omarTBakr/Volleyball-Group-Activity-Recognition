...
\begin{table}[h]
    \centering
    \caption{Baseline models. B1--B6 complete with results; B7--B8 implemented (B7 has one result from a superseded config, B8 untrained).}
    \label{tab:baselines}
    \begin{tabular}{lccc}
        \toprule
...
\begin{table}[h]
    \centering
    \caption{Baseline 7 test-set metrics --- \emph{superseded config} (see text).}
...
These numbers are from an \textbf{initial single-phase run with the player model frozen} --- they land \emph{below} B6 (70.5\%) and even B5, the same ``frozen pretrained temporal weights cap performance'' lesson B6's run~1 taught. The code has since moved to the two-phase probe~$\to$~joint fine-tune scheme (unfreezing LSTM$_1$) plus AdamW, which is the run that will show whether the hierarchy pays off; it has not been re-run. B7 still pools all players side-blind, so its confusion matrix keeps the left/right leakage --- unaddressed until B8.
...
B8 is implemented and shape-verified (forward/backward for both pool modes, checkpoint round-trip, the zero-player-team edge case, and Stage~A ignoring \texttt{team\_ids} all pass a synthetic test) but \textbf{not yet trained}. It is the ablation rung that isolates team structure: in the extension paper, team-split pooling is worth \textbf{$+11.6$ points ($70.3 \to 81.9$)} over all-player pooling, and it targets exactly the left/right errors the confusion matrices have shown since B5. One caveat: quality hinges on the loader's $x$-coordinate team assignment, which can mislabel players near mid-court --- if B8 does not beat B7, inspect the split before the pooling.

\section{Next Steps}

(i) \textbf{Re-run B7} under the two-phase (probe $\to$ joint fine-tune) + AdamW config that superseded its frozen single-phase result --- unfreezing LSTM$_1$ is what closed B6's probe/fine-tune gap, and B7 has more pretrained capacity to unlock. (ii) \textbf{Train and evaluate B8} --- its team-split pooling is the direct fix for the left/right winpoint/pass/set collapse diagnosed since B5, and the paper's largest single ablation gain. (iii) Reduce overfitting across all baselines --- stronger augmentation, higher dropout, and earlier stopping remain the levers (B6's fine-tune still reaches ${\sim}93\%$ train F1 against ${\sim}64\%$ validation).
