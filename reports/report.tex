...
\begin{abstract}
   A condensed summary of the Volleyball Group Activity Recognition project: the data pipeline, the generic multi-baseline loader, and the two completed baselines --- B1, a single-frame fine-tuned ResNet-50 (62.6\% test accuracy / 0.630 macro F1), and B3, a person-then-group crop classifier (60.7\% / 0.589). The dataset (Ibrahim et al., CVPR 2016) contains 55 videos / 4{,}830 clips with 8 scene-level group activities and 9 player-level actions.
\end{abstract}
...
(i) Reduce overfitting in both baselines (stronger augmentation, higher dropout, earlier stopping); (ii) proceed to the temporal baselines B4--B8 --- noting that player tracking covers only $\approx$half of each clip's frames, so the temporal crop baselines will exercise the loader's detection-fallback path, which should be reviewed before B5.
